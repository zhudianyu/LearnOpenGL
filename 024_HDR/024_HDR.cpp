...
int main()
{
	...
	string vertName("../" + projName + "/" + projName + ".vert");
	string fragName("../" + projName + "/" + projName + ".frag");
	Shader lightShader("../024_HDR/lighting.vert", "../024_HDR/lighting.frag");
	Shader hdrShader(vertName.c_str(),fragName.c_str());
	...
	{
		cout << "frame buffer not complete " << glCheckFramebufferStatus(GL_RENDERBUFFER) << endl;
	}
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	float offset = -10;
	//光源信息
	vector<glm::vec3> lightPositions;
	lightPositions.push_back(glm::vec3(0, 0, 48.5f+offset));
	lightPositions.push_back(glm::vec3(-1.4f, -1.9f, 9.0f + offset));
	lightPositions.push_back(glm::vec3(0,-1.8f,4.0f + offset));
	lightPositions.push_back(glm::vec3(0.8f,-1.7f,6.0f + offset));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(0, 0, 25.0f + offset/2));
